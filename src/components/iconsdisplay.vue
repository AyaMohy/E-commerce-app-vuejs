...
function addtowishlist(){
    let targetitem=quantityincart.value.itemswishlist.findIndex((i)=>i.id==item.value.id )
    if(targetitem !=-1){
        quantityincart.value.itemswishlist.splice(targetitem,1)
    }else{
        quantityincart.value.additemstowishlist(item.value) 
    }
...
<template>
    <div>
        <!-- iconsdisplay -->
        <button   class="btn d-block  text-white" @click.once="(quantityincart.increasecount(1)),(isactive= !isactive)"> 
        ...
        <button :class="{disabled: isactive2 }" class="btn text-white" @click="(addtowishlist()),(isactive2= !isactive2)">
    ...
    <!-- <p class="bg-danger">id: {{increaseoneitem()}}</p> -->
</template>
